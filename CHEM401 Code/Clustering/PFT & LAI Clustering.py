...
# Set PFT type, number of clusters, and month
...
# normalise variables (robust scalar due to presence of outliers and minmax scalar to normalise with PFT data)

LAI = preprocessing.RobustScaler().fit_transform(LAI)  
LAI = preprocessing.MinMaxScaler().fit_transform(LAI) 



# Use PFT type set at start and combine PFT and LAI data together
...
PFT_type = np.hstack((PFT_type,LAI[:,0:2]))
...
plt.savefig('PFT with LAI/' + PFT_name + '/' + month_text + ' PFT_LAI_Cluster Map k=' + str(no_clusters) +' ' + PFT_name + '.png')
plt.close()
print('PFT_LAI_Cluster Map.png saved')
