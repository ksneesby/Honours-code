...
# CHANGE THESE IF YOU NEED TO: they are the parent folders of the swaths and fires
__fires_path__  = '/short/m19/satellite-data/MOD14A1/'
__omi_path__    = '/short/m19/satellite-data/aura/omi/OMHCHOv003/'
...
def create_lat_lon_grid(latres,lonres):
    '''
...
def read_key(date,dateN=None, key='VC_C', firethresh=-1, boundary=None):
    '''
...
def read_omhcho(path, szamax=60, screen=[-5e15, 1e17], maxlat=60):
    '''
...
def read_MOD14A1(date=datetime(2005,1,1), per_km2=False):
    ...
    fpath=__fires_path__+date.strftime('%Y/MOD14A1_D_FIRE_%Y-%m-%d.CSV')
...
def get_good_pixel_list(date, maxlat=60):
    ...
    fpattern=date.strftime(__omi_path__+'%Y/OMI-Aura_L2-OMHCHO_%Ym%m%d*')
    files = glob(fpattern)
    assert len(files) > 0, "omhcho data is not at %s"%fpattern
...
def make_gridded_swaths(date, latres=__LATRES__, lonres=__LONRES__, remove_clouds=True):
    '''
    ...
    if __VERBOSE__:
        print(np.sum(cloud_filter),' cloudy pixels removed!')
    ...
    if __VERBOSE__:
        print("Starting to put pixels together now...")
    ...
    if __VERBOSE__:
        print("Pixels should now be gridded, next step saving...")
    ## 5)
    ...
    save_to_hdf5(outfilename, outd, attrdicts=attrs)
    if __DEBUG__:
        print("File should be saved now...")



def regrid_N_days(date, latres=__LATRES__, lonres=__LONRES__, remove_clouds=True,
                     days=8, processes=8):
    '''
...
def plot_map(data,lats,lons, linear=True,
             region=__AUSREGION__,vmin=None,vmax=None,
             cbarlabel='',cmap=None):
    '''
    '''
    ...
    if not linear:
        norm=LogNorm()
    cs=m.pcolormesh(mlons, mlats, data, latlon=True,
                    norm=norm,
    ...
    # add colour bar if label exists
    if not (cbarlabel==''):
