...
LAI_centres = np.full((6,4),np.nan)

LAI_centres[2,:] = (-0.0648321,-0.223288,0.0645077,-0.643086)
LAI_centres[1,:] = (3.96158,2.63939,-0.440223,0.710267)
LAI_centres[4,:] = (1.17668,1.67736,-0.137557,0.70708)
LAI_centres[3,:] = (8.41311,3.13272,-0.928003,0.654017)
LAI_centres[5,:] = (1.25443,0.602805,0.990831,0.0210763)
LAI_centres[0,:] = (-0.108942,-0.0387478,-0.336281,0.266105)

 
kmeans = KMeans(n_clusters=no_LAI, init=LAI_centres)
kmeans = kmeans.fit(LAI)
labels = kmeans.predict(LAI)
C_LAI = kmeans.cluster_centers_
...
L_LAI_reshape = np.reshape(L_LAI,(len(no_ocean_lats),len(no_ocean_lons)))
copy_LAI_reshape = copy(L_LAI_reshape)
...
L_LAI_reshape[(L_LAI_reshape!=4) & (~np.isnan(L_LAI_reshape))] = 1
...
plt.subplot(235)
rs.plot_map(diff,no_ocean_lats,no_ocean_lons,linear=True,
            vmin=-4,vmax=4,cbarlabel='diff = '+ str(np.nansum(diff)), cmap="tab10")
...
L_LAI_reshape = copy(copy_LAI_reshape)

L_HCHO_reshape[(L_HCHO_reshape!=2) & (~np.isnan(L_HCHO_reshape))] = 5
L_LAI_reshape[(L_LAI_reshape!=2) & (~np.isnan(L_LAI_reshape))] = 4
...
L_HCHO_reshape[(L_HCHO_reshape!=5) & (~np.isnan(L_HCHO_reshape))] = 3
L_LAI_reshape[(L_LAI_reshape!=5) & (~np.isnan(L_LAI_reshape))] = 2
...
plt.savefig('closeup/LAI/LAI difference map closeup '+month_text+' K=6.png')
plt.close()
