...
no_EF = 6
no_HCHO = 6
no_clusters = max(no_EF,no_HCHO)       ### Number of clusters, use highest K
...
EF_centres = np.full((6,3),np.nan)

EF_centres[2,:] = (-0.0813693,-0.741354,-0.248518)
EF_centres[3,:] = (-0.368031,0.641688,0.761771)
EF_centres[0,:] = (-0.477716,0.269479,-0.666224)
EF_centres[5,:] = (0.633455,-0.0274913,0.0455999)
EF_centres[1,:] = (1.06091,0.117916,1.12768)
EF_centres[4,:] = (-0.959878,0.284875,2.53143)


kmeans = KMeans(n_clusters=no_EF,init=EF_centres)
kmeans = kmeans.fit(X)
labels = kmeans.predict(X)
C_EF = kmeans.cluster_centers_
L_EF = kmeans.labels_


L_EF = np.array(L_EF, dtype=float)
for i in range(len(to_delete)):
    if np.isfinite(to_delete[i]):
        L_EF = np.insert(L_EF,to_delete[i].astype(int),np.nan)
L_EF_reshape = np.reshape(L_EF,(len(no_ocean_lats),len(no_ocean_lons)))


copy_EF_reshape = copy(L_EF_reshape)
...
rs.plot_map(L_EF_reshape,no_ocean_lats,no_ocean_lons,linear=True,
            vmin=0,vmax=no_clusters,cbarlabel="", cmap="tab10")
plt.show

L_HCHO_reshape[(L_HCHO_reshape!=4) & (~np.isnan(L_HCHO_reshape))] = 2
L_EF_reshape[(L_EF_reshape!=4) & (~np.isnan(L_EF_reshape))] = 1

diff = L_HCHO_reshape-L_EF_reshape
# =============================================================================
# diff[np.isnan(diff)] = 999
# diff[(diff != 0) & (diff <100)] = 1
# diff[diff == 999] = np.nan
...
L_EF_reshape = copy(copy_EF_reshape)

L_HCHO_reshape[(L_HCHO_reshape!=2) & (~np.isnan(L_HCHO_reshape))] = 5
L_EF_reshape[(L_EF_reshape!=2) & (~np.isnan(L_EF_reshape))] = 4
...
L_EF_reshape[(L_EF_reshape!=1) & (~np.isnan(L_EF_reshape))] = -2
...
plt.savefig('EF/closeup/EF difference map closeup '+month_text+' K=6.png')
plt.close()
