...
kmeans = KMeans(n_clusters=no_SM)
kmeans = kmeans.fit(X)
...
kmeans.labels_[np.isnan(kmeans.labels_)] = no_SM
kmeans.labels_ = kmeans.labels_.astype(int)


all_lut = list(itertools.permutations(np.arange(0,no_SM,1)))

for i in range(len(all_lut)):
    temp = all_lut[i]
    temp = np.array(temp, dtype=int)
    temp = np.append(temp,np.nan)
    temp_SM = temp[kmeans.labels_]
    temp_SM = np.array(temp_SM, dtype=float)
    
    temp_SM_reshape = np.reshape(temp_SM,(len(no_ocean_lats),len(no_ocean_lons)))
    temp_diff = L_HCHO_reshape-temp_SM_reshape
...
plt.savefig('Soil Moisture/Difference Plots/' + month_text + ' HCHO-SM Difference Map (K=' + str(no_HCHO) + ', ' + str(no_SM) + ').png')
plt.close()
