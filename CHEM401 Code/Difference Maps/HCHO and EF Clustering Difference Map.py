...
kmeans = KMeans(n_clusters=no_EF)
kmeans = kmeans.fit(X)
...
kmeans.labels_[np.isnan(kmeans.labels_)] = no_EF
kmeans.labels_ = kmeans.labels_.astype(int)


all_lut = list(itertools.permutations(np.arange(0,no_EF,1)))

for i in range(len(all_lut)):
    temp = all_lut[i]
    temp = np.array(temp, dtype=int)
    temp = np.append(temp,np.nan)
    temp_EF = temp[kmeans.labels_]
    temp_EF = np.array(temp_EF, dtype=float)
    
    temp_EF_reshape = np.reshape(temp_EF,(len(no_ocean_lats),len(no_ocean_lons)))
    temp_diff = L_HCHO_reshape-temp_EF_reshape
...
plt.savefig('EF/Difference Plots/' + month_text + ' HCHO-EF Difference Map (K=' + str(no_HCHO) + ', ' + str(no_EF) + ').png')
plt.close()
